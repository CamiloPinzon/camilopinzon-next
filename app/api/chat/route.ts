...
function getRateLimiter(): Ratelimit | null {
  ...
  if (
    ...
    console.warn(
      "[API Chat] Upstash env vars not set — rate limiting disabled.",
    );
    return null;
  }
...
export async function GET() {
  return new Response("API Chat Route is fully operational and reachable!", {
    status: 200,
  });
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (limiter) {
      ...
      console.log(
        `>>> [API Chat] Rate limit — IP: ${ip} | remaining: ${remaining}`,
      );
      ...
      if (!success) {
        ...
        return new Response(
          JSON.stringify({
            ...
            message:
              "Has enviado demasiadas preguntas. Por favor espera un momento e intenta de nuevo.",
    ...
    // Format experience
    const experiences = experienceSnap.docs
      .map((doc) => {
        const data = doc.data();
        const localized =
